...
%
% A tab separated warning file, if there is a conflict. The following
% information are recorded: x,y,z and i,j,k indices of voxels in conflict;
% maximum probability at that voxel, the indices of regions in conflict at
% that location, the index of region assigned to that voxel, and a record
% of how the conflict was resolved: 'eliminated' for voxels which were
% eliminated, 'three' for voxels whose conflict was resolved by a
% neighbourhood search of 3x3x3, 'five' for voxels whose conflict was
% resolved by a neighbourhood search of 5x5x5, and 'random' for voxels
% which were randomly allocated a region (see Notes)
%
% A summary file, if there is a conflict, mentioning the number of voxels
% in conflict, the number of voxels whose conflict were resolved by
...
if nargin == 1
    ...
    summary_file_name   = fullfile(atlas_path, [atlas_name, '_maxprob_thr', ...
        num2str(threshold), '_summary.txt']);
    ...
    if nargin == 2
        ...
        if nargin == 3
            ...
            if isempty(output_file_loc)
                ...
                summary_file_name   = fullfile(atlas_path, [atlas_name, ...
                    '_maxprob_thr', num2str(threshold), '_summary.txt']);
                ...
                warning_file_name = fullfile(output_file_loc, [atlas_name, ...
                    '_maxprob_thr', num2str(threshold), '_warnings.txt']);
                summary_file_name   = fullfile(output_file_loc, [atlas_name, ...
                    '_maxprob_thr', num2str(threshold), '_summary.txt']);
...
% Open a file to write warnings and print header
fid_warn = fopen(warning_file_name, 'w');
fprintf(fid_warn, '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\r\n', ...
    'x', 'y', 'z', 'i', 'j', 'k', 'max_prob', 'which_regions', ...
    'assigned_region', 'resolution_method');
    ...
    if prob_val(i) ~= 0
        ...
        if tmp > 1
            ...
            if sum(ismember(count_tab(:,1), which_regions'))==0
                ...
                roi_idx(i) = 0;
                % Report
                warning_msg = sprintf('%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s', ...
                    num2str(atlas_xyz(i,1)), ...
                    num2str(atlas_xyz(i,2)), ...
                    num2str(atlas_xyz(i,3)), ...
                    num2str(atlas_ijk(i,1)), ...
                    num2str(atlas_ijk(i,2)), ...
                    num2str(atlas_ijk(i,3)), ...
                    num2str(prob_val(i)), ...
                    num2str(which_regions, '%02d, '), ...
...
if warn_count > 0
    msg = ['Total number of voxels with conflicting maximum probability = ', ...
        num2str(warn_count)];
    disp(msg);
    % Open the summary file and write details
    fid_summary = fopen(summary_file_name, 'w');
